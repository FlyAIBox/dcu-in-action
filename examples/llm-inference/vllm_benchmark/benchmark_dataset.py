...
"""
benchmark_dataset.py - 基准测试数据集处理模块

本模块定义了一个框架，用于从各种数据集中采样基准测试请求。每个BenchmarkDataset的子类
都必须实现样本生成功能。主要功能包括：

1. 多数据集支持：支持多种类型的数据集，包括ShareGPT、随机生成、Sonnet诗歌、
   BurstGPT、HuggingFace数据集、VisionArena等
2. 多模态支持：支持文本、图像、音频等多种模态的数据处理
3. LoRA支持：支持Low-Rank Adaptation参数的随机选择和应用
4. 请求采样：提供灵活的请求采样策略，支持过采样和数据增强
5. 数据验证：确保生成的请求符合模型的输入要求

支持的数据集类型：
- ShareGPT：对话数据集，包含多轮对话内容
- RandomDataset：合成随机数据，用于压力测试
- SonnetDataset：莎士比亚十四行诗数据集
- BurstGPTDataset：突发性文本生成数据集
- HuggingFaceDataset：HuggingFace平台上的各种数据集
- VisionArenaDataset：视觉对话竞技场数据集（多模态）
- ConversationDataset：通用对话数据集（支持多模态）
- InstructCoderDataset：代码编辑指令数据集
- MTBenchDataset：多任务基准测试数据集
- AIMODataset：数学推理问题数据集
- NextEditPredictionDataset：下一步编辑预测数据集
- ASRDataset：自动语音识别数据集（音频转文本）
...
class SampleRequest:
    ...
    表示一个用于基准测试的推理请求，包含输入提示、长度信息、
    多模态数据和LoRA适配器等所有必要信息。
    """
    prompt: Union[str, Any]                                              # 输入提示词（文本或其他格式）
    ...
    lora_request: Optional[LoRARequest] = None                           # LoRA适配器请求（可选）
...
class BenchmarkDataset(ABC):
    ...
    定义了所有数据集实现必须遵循的接口和通用功能。子类必须实现
    load_data()和sample()方法来提供特定数据集的加载和采样逻辑。
    """
    ...
    def __init__(
        ...
        Args:
            dataset_path: 数据集路径（可选）。如果为None，表示可能使用
                         默认数据集或随机生成的数据集
            random_seed: 用于可重现的洗牌或采样的随机种子值，
                        默认值为DEFAULT_SEED
        """
