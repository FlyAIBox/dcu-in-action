...
class RequestFuncInput:
    ...
    这个数据类定义了向大模型推理后端发送请求时需要的所有参数。
    它提供了统一的接口，支持多种不同的推理后端 (vLLM, TGI, OpenAI等)。
...
class RequestFuncOutput:
    ...
    主要功能:
    1. 存储生成的文本内容和元数据
    2. 记录详细的性能指标 (TTFT, ITL等)
    3. 提供错误处理和状态跟踪
    4. 支持流式响应的时间戳记录

    性能指标说明:
    - latency: 总请求延迟时间
    - ttft: 首次token时间 (Time To First Token)
    - itl: 迭代延迟列表 (Inter-Token Latency)
    - prompt_len: 实际输入长度
    - output_tokens: 实际输出token数量
    """
